...
export function renderSection(s: Section, base: string = '', lang: 'en' | 'zh' = 'en'): string {
  const ctaLabel = lang === 'zh' ? '提交您的视频' : 'Submit Your Video';
  switch(s.type) {
    ...
    case 'cta': return `<div class="article-cta"><p>${s.content}</p><a href="${base}/submit/" class="btn btn-primary">${ctaLabel} <span class="chev">›</span></a></div>`;
    case 'ul': return `<ul class="article-ul">${(s.items||[]).map((item:string) => `<li>${item}</li>`).join('')}</ul>`;
    case 'ol': return `<ol class="article-ol">${(s.items||[]).map((item:string) => `<li>${item}</li>`).join('')}</ol>`;
    default: return '';
  }
}
